...
# 获取测试数据
data=MiddleHandler.case_path.read_sheet("email")
...
class TestUsername(unittest.TestCase):
    ...
    def test_username(self,test_info):
        ...
        if "#email#" in test_info["excepted"]:
            ...
            test_info["excepted"] = test_info["excepted"].replace("#email#",(data_info.split("/"))[2])
        # 如果预期结果中有#pass_username#,替换生成的pass_username
        if "#pass_email#" in test_info["excepted"]:
            test_info["excepted"] = test_info["excepted"].replace("#pass_email#", (data_info.split("/"))[2])
        # 断言
        try:
